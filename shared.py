...
WORKTREE_NAME = os.environ.get("LARYNX_WORKTREE_NAME", os.path.basename(BASE_DIR))
...
def evaluate_hierarchical(model, loader, device, num_classes):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            all_is_voc_true.extend(is_voc.numpy() if hasattr(is_voc, 'numpy') else is_voc)
...
# ── visualisation ────────────────────────────────────────────────────────────
def save_training_curves(history_df, best_epoch, supcon_history=None):
    ...
    ax_f1.set_title("Phase 2 — Macro F1", fontsize=14, fontweight="bold",
                     pad=12, color="#1F2937")
    ...
    ax_acc.set_title("Phase 2 — Accuracy", fontsize=14, fontweight="bold",
                      pad=12, color="#1F2937")
    ...
    ax_auc.set_title("Phase 2 — AUROC", fontsize=14, fontweight="bold",
                      pad=12, color="#1F2937")
    ...
    title = "Training Curves (Phase 1 + Phase 2)" if has_supcon else "Training Curves"

...
`视频识别/video_inference.py` 将喉镜视频当作抽帧后的图像集合处理，不训练时序模型。脚本会按固定 fps 抽帧，应用与训练一致的黑边裁切、Resize、CenterCrop 和 ImageNet normalize，批量送入现有图像 checkpoint；随后输出帧级概率、视频级弱监督聚合结果、候选有效片段和关键帧。

视频数据已移出代码 workspace，统一放在 `/mnt/data/LarynxData/videos/`。脚本默认读取 `/mnt/data/LarynxData/videos/ali_larynx_20260505`；也可用 `LARYNX_VIDEO_ROOT=/path/to/videos` 或 `--video-root` 覆盖。

视频诊断逻辑默认分四步：先用质量阈值过滤明显无效帧，再用 `图像识别/glottis_binary/` 训练出的二分类 Swin gate 筛出声门/有效声带帧，然后在保留下来的帧中用 8 分类模型做疾病概率预测，最后对各病种概率做 top-fraction 聚合。`diagnosis_summary.csv` 采用 `--diagnosis-variant` 指定的规则（默认 `voc_sum_gt_nonvoc`）生成病人级结论，并记录该病种高概率出现的秒段。

默认二分类 gate 使用 `Results/main/glottis_binary_benchmarks/20260505_183333_parallel/swin_base/best_model.pth`，主阈值为 `prob_glottis >= 0.94`，默认不自动降阈值，以优先减少非声门帧进入疾病分类。`--max-segment-gap-sec` 默认 `1.0` 秒，用于把闪光、瞬时模糊等短暂中断前后的声门片段合并为同一候选区间；如确实需要提高召回，可显式设置 `--glottis-gate-fallback-threshold 0.62`。可用 `--no-glottis-gate` 回到旧的弱 VOC/Non-VOC gate。
...
python 视频识别/video_inference.py \
  --video-root /mnt/data/LarynxData/videos/ali_larynx_20260505 \
  --model ../../Results/selective_unfreeze_confusion10/best_model.pth \
  --output-dir ../../Results/main/video_inference_ali_20260505_selective_unfreeze_corrected_labels \
  --sample-fps 8 \
...
脚本仍会在二分类 gate 保留下来的帧内比较多种候选区间规则：全二分类合格帧、`sum(VOC 概率) > Non-VOC 概率`、更严格的 VOC margin，以及低 Non-VOC 阈值。这里 8 分类模型里的 Non-VOC 只作为第二层弱筛选信号，不被解释为严格的“非声带/声带”二分类器。视频级类别分数默认取候选帧中该类概率最高的前 20% 求均值，避免普通多数投票被大量重复或无效角度稀释。
...
| 文件 / 目录 | 说明 |
|------|------|
| `frame_predictions.csv` | 每个抽样帧的质量指标、二分类 `prob_glottis`/gate 结果、8 分类 argmax、Non-VOC 概率、VOC 概率和、逐类概率 |
| `video_predictions.csv` | 每个视频在不同 gate 规则下的预测类别、分数、候选片段长度和逐类 top-fraction 分数 |
| `video_segments.csv` | gate 片段与预测类别高置信证据片段 |
| `summary.json` | 各 gate 规则的视频级准确率和预测分布 |
